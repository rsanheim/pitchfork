...
git tag v2.15.0-rsanheim.3 <full-commit-sha>
git push origin v2.15.0-rsanheim.3
my-crow pipeline create rsanheim/pitchfork --tag v2.15.0-rsanheim.3
```
...
```sh
PITCHFORK_RELEASE_DIR=/path/to/releases script/release upload v2.15.0-rsanheim.3 linux
```

On a native Apple Silicon Mac, check out the **same tag**, then:

```sh
script/release build v2.15.0-rsanheim.3 macos
script/release upload v2.15.0-rsanheim.3 macos
```

After checking the artifacts, publish explicitly:

```sh
gh release edit v2.15.0-rsanheim.3 --repo rsanheim/pitchfork \
...
```toml
"github:rsanheim/pitchfork" = "2.15.0-rsanheim.3"
```
